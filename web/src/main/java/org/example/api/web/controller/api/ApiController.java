...
public class ApiController {
    ...
    public PageResult page(@Parameter(name = "page", description = "页码", in = ParameterIn.QUERY, required = true)
                           @RequestParam(name = "page", defaultValue = "1")
                                   Long page,
                           @Parameter(name = "limit", description = "大小", in = ParameterIn.QUERY, required = true)
                           @RequestParam(name = "limit", defaultValue = "10")
                                   Long limit,
                           @Parameter(name = "type", description = "类型", in = ParameterIn.QUERY, required = false)
    ...
    public Result login(
            @Parameter(name = "pwd", description = "密码", in = ParameterIn.QUERY, required = true) String pwd,
            @Parameter(name = "email", description = "邮箱", in = ParameterIn.QUERY, required = true)
                    String email) {
        ...
        if (one != null) {
            if (one.getState() == 0) {
                ...
                return Result.ok(JwtUtils.getToken(stringStringHashMap));
            }
    ...
    public Result upload(@Parameter(description = "图片文件", in = ParameterIn.QUERY, required = true)
                         @RequestParam("file") MultipartFile file,
                         @Parameter(description = "保存模式", in = ParameterIn.PATH, required = true)
                         @PathVariable int save_mode) throws IOException {
    ...
    public Result update_avatar(@Parameter(hidden = true) @Userid String id,
                                @Parameter(description = "图片链接 上传文件后返回的链接", in = ParameterIn.QUERY, required = true)
                                        String link) {
        User user = new User();
    ...
    public Result activation_vip(@Parameter(hidden = true) @Userid String id,
                                 @Parameter(description = "激活码", in = ParameterIn.QUERY, required = true)
                                         String code) {
    ...
    @GetMapping("/user/ad")
    public Result ad_list(
            @Parameter(description = "广告类型", in = ParameterIn.QUERY, required = false)
            @RequestParam(name = "type", defaultValue = "-1") Integer type,
            @Parameter(description = "指定标识", in = ParameterIn.QUERY, required = false)
    ...
    public PageResult provide_list(@Parameter(name = "page", description = "页码", in = ParameterIn.QUERY, required = true)
                                   @RequestParam(name = "page", defaultValue = "1")
                                           Long page,
                                   @Parameter(name = "limit", description = "大小", in = ParameterIn.QUERY, required = true)
                                   @RequestParam(name = "limit", defaultValue = "10")
    ...
    @GetMapping("/user/provide/pay")
    public Result pay(@RequestBody Order order, @Parameter(hidden = true) @Userid String uid) {
    ...
    public PageResult now(@Parameter(name = "page", description = "页码", in = ParameterIn.QUERY, required = true)
                          @RequestParam(name = "page", defaultValue = "1")
                                  Long page,
                      @Parameter(name = "limit", description = "大小", in = ParameterIn.QUERY, required = true)
        ...
        promoLambdaQueryWrapper.ge(Promo::getBeginTime,LocalDateTime.now())
                .le(Promo::getEndTime,LocalDateTime.now());
    ...
    public Result query_promo(@Parameter(name = "pid", description = "商品id", in = ParameterIn.QUERY, required = true)
    String pid,@Parameter(name = "vip_type", description = "vip类型", in = ParameterIn.QUERY, required = true)
                          @RequestParam(name = "vip_type", defaultValue = "1")
                          Integer vip_type,
                                  @Parameter(hidden = true)@Userid String uid
                   ) {
        LambdaQueryWrapper<Promo> promoLambdaQueryWrapper = new LambdaQueryWrapper<>();
